...
class EntityMatcher extends ConfigurableMatcherBase {
  ...
  public function execute($string) {
    ...
    $url = $this->resolveEntityUrl($string);
    if ($url) {
      $result = [$url->getRouteParameters()[$this->targetType]];
    ...
    foreach ($entities as $entity) {
      ...
      $suggestion = $this->createSuggestion($entity, $options);
  ...
  protected function createSuggestion(EntityInterface $entity, array $options = []) {
    ...
    $suggestion->setLabel($this->buildLabel($entity))
      ...
      ->setPath($this->buildPath($entity, $options));
  ...
  protected function buildPath(EntityInterface $entity, array $options = []) {
    ...
    $path = $entity->toUrl('canonical', $options)->toString();
  ...
  /**
   * Finds entity url from the given input.
   *
   * @param string $user_input
   *   The string to url parse.
   *
   * @return \Drupal\Core\Url|null
   *   An url object for the entity the user input.
   */
  protected function resolveEntityUrl(string $user_input): ?Url {
    try {
    ...
    return $url->isRouted() && $url->getRouteName() === "entity.$this->targetType.canonical"
      ? $url
      : NULL;
  }
   ...
   *
   * @deprecated Use ::findEntityUrl() instead.
   */
  protected function findEntityIdByUrl($user_input) {
    $url = $this->resolveEntityUrl($user_input);

    return $url ? $url->getRouteParameters()[$this->targetType] : NULL;
  }
